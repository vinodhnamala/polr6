        ...
        for (let lpmVal = lpmInitValue; true; lpmVal = lpmVal + (2 * increment)) {
            ...
            for (let mn = 0; mn < rowrangeopt.length; mn++) {
                ...
                for (let j = 0; j < (rowrangeopt.length * 10); j++) {
                    ...
                    //Extent changes   grgroundtruth_txtlpmnoExtent_0

                    var lpmExtentTextId = '#' + savebtnId.replace('grgroundtruth_btnsave_', 'grgroundtruth_txtlpmnoExtent_');
                    await page.waitForSelector(lpmExtentTextId, { timeout: 0 });
